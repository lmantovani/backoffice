...
class PurchaseOrderClosureAPITests(APITestCase):
    def setUp(self):
        self.url_encerrar = reverse('po-closure-encerrar')
        self.url_reprocessar = reverse('po-closure-reprocessar-falhas')

    @patch('purchase_orders.services.OmieAPIClient')
    def test_encerrar_pedido_success_sync(self, MockClient):
        instance = MockClient.from_settings.return_value
        ...
        instance.consultar_pedido_compra.assert_called_once_with({'cNumero': 'PO123'})
